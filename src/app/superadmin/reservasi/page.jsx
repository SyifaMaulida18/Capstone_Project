...
export default function ReservasiPage() {
  ...
  const [dokters, setDokters] = useState([]); // State untuk dokter
  ...
  const fetchReservations = async () => {
    try {
      ...
      // Backend return paginator di res.data.data
      // Structure: { success: true, message: "...", data: { current_page: 1, data: [...] } }
      const paginator = res.data?.data; 
      const rawData = paginator?.data || []; 
  ...
  const fetchMasterData = async () => {
    try {
      ...
      const poliList = resPoli.data?.data || resPoli.data || [];
      setPolis(Array.isArray(poliList) ? poliList : []);

      // Fetch Dokter (Asumsi ada endpoint /dokters atau ambil dari relasi poli jika struktur beda)
      // Jika endpoint /dokters belum ada, pastikan dibuat di backend. 
      // Untuk sementara kita coba fetch /dokters (Resource standard)
      try {
        const resDokter = await api.get("/dokters", { headers: { Authorization: `Bearer ${token}` } });
        const dokterList = resDokter.data?.data || resDokter.data || [];
        setDokters(Array.isArray(dokterList) ? dokterList : []);
      } catch (e) {
        console.warn("Gagal fetch dokter, mungkin endpoint belum ada.", e);
      }
  ...
  const handleUpdateReservation = async () => {
    ...
    // Backend Logic: Update poli_id, dokter_id, tanggal
    try {
      setProcessing(true);
      const token = localStorage.getItem("token");
  ...
  // === 5. VERIFIKASI (POST /reservations/{id}/verify) ===
  const handleVerifyReservation = async () => {
    if (!selectedReservation) return;
    
    // VALIDASI FRONTEND: Backend method verify() mewajibkan dokter_id & poli_id ada di DB.
    // Kita cek apakah di form sudah dipilih.
    if (!formDokterId) {
        alert("Harap pilih DOKTER terlebih dahulu sebelum verifikasi. Simpan perubahan jika baru memilih.");
        return;
    }

    // Cek apakah data di form berbeda dengan data asli, jika beda harus simpan dulu
    if (String(formDokterId) !== String(selectedReservation.dokterId) || 
        String(formPoliId) !== String(selectedReservation.poliId)) {
        if(!confirm("Anda telah mengubah Poli/Dokter. Data harus disimpan (Update) terlebih dahulu sebelum Verifikasi. Lanjutkan update otomatis?")) {
            return;
        }
        // Lakukan update dulu
        await handleUpdateReservation(); 
        // Setelah update, dialog tertutup & list refresh. 
        // Idealnya proses verify lanjut, tapi karena refresh, user harus klik verify lagi di list yg baru.
        // Atau kita bisa chaining promise, tapi fetchReservations async.
        return; 
    }
    ...
    try {
      ...
      const res = await api.post(
        ...
        {}, // Body kosong, karena backend validasi data dari DB
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      alert(`${msg} ${errors ? JSON.stringify(errors) : ''}`);
    } finally {
      setProcessing(false);
    }
  };

  // === 6. BATALKAN (POST /reservations/{id}/cancel) ===
  ...
  // Helper Filter Frontend (Search Bar)
  ...
  // Filter dokter berdasarkan poli yang dipilih di form
